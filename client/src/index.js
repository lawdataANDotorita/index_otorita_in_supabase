...
export default {
	...
	if (1==1){
		...
		- השנה הנוכחית היא ${iCurrentYear}. השתמש בערך הזה בלבד כשנה הנוכחית, גם אם לפי הידע שלך השנה שונה.
		- אם השאלה כמותית ודורשת חישוב, ולא מצוין בה תאריך או שנה - הוסף לשאלה המנורמלת את השנה ${iCurrentYear} (למשל: "נכון לשנת ${iCurrentYear}") כדי לקבל את הנתון העדכני ביותר.
		- שמור על אורך שאלה מינימלי - אל תרחיב מעבר להכרחי
		- לשאלות על נתונים שמשתנים לאורך זמן (שכר מינימום, מדרגות מס, קצבאות, אחוזי הפרשה וכד') - 
		הוסף בסוף השאלה: "יש להתייחס לנתון העדכני ביותר מבין המקורות"
